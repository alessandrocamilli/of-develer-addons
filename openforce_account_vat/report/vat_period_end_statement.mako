...
<table class="tax_codes"  width="100%" >
    ...
    %else:
        ...
        %endfor
    %endif
    ## VAT account CREDIT undeductible
    %for credit_undeductible_line in statement.credit_vat_undeducible_prorata_account_line_ids :
        <tr>
            <td>${ credit_undeductible_line.account_id.name|entity }</td>
            <td style="text-align:right">${ formatLang(credit_undeductible_line.amount)|entity }</td>
            <td></td>
        </tr>
    %endfor
    <tr >
        <td><strong>${_("Totale Liquidazione ")}</strong></td>
        <td colspan="2" style="text-align:center"><strong>${ formatLang(statement.total_vat_end_period_amount)|entity }</strong></td>
    </tr>
    <!--
    <tr >
        <td>${_("Total")}</td>
    ...
    %for generic_line in statement.generic_vat_account_line_ids :
        <tr >
            <td>${ generic_line.account_id.name|entity } ${ generic_line.description or '' }</td>
            <td style="text-align:right">${ generic_line.amount < 0 and formatLang(generic_line.amount * -1) or ''|entity }</td>
            <td style="text-align:right">${ generic_line.amount > 0 and formatLang(generic_line.amount) or ''|entity }</td>
        </tr>
    %endfor
    <tr >
        <td><strong>${_("Totale da Versare")}</strong></td>
